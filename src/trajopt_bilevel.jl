function traj_fn_bilevel(traj_data)
    ...
    if (traj_data.num_contacts > 0)
        c_n_selector = last_selector .+ (1:traj_data.num_contacts)
    ...
    g_kin_selector = 1:traj_data.num_q
    g_dyn_selector = g_kin_selector[end] .+ (1:traj_data.num_v)
    g_eq_selector = vcat(g_kin_selector,g_dyn_selector)
    g_ineq_selector = []
    if (traj_data.num_contacts > 0)
        g_dist_selector = g_dyn_selector[end] .+ (1:traj_data.num_contacts)
        g_norm_selector = g_dist_selector[end] .+ (1:traj_data.num_contacts)
        g_comp_selector = g_norm_selector[end] .+ (1:traj_data.num_contacts)
    ...
    function eval_dyn(x::AbstractArray{T}) where T
        ...
        if (traj_data.num_contacts > 0)
            ...
            for i = 1:traj_data.num_contacts
                rel_transforms[i] = (relative_transform(xnext, traj_data.obstacles[i].contact_face.outward_normal.frame, traj_data.world_frame),relative_transform(xnext, traj_data.contact_points[i].frame, traj_data.world_frame))
        ...
        if (traj_data.num_contacts > 0)
            contact_force, contact_x, contact_λ, contact_μ = solve_contact_τ(traj_data,H,dyn_bias,rel_transforms,geo_jacobians,v0,u0,c_ns,ip_method=false,in_place=false)
        else
            contact_force = zeros(traj_data.num_v)
        end

        g = zeros(T,traj_data.num_dyn_eq+traj_data.num_dyn_ineq)

        g[g_kin_selector] = qnext .- q0 .- traj_data.Δt .* config_derivative
        g[g_dyn_selector] = H * (vnext - v0) .+ traj_data.Δt*(dyn_bias .- u0 .+ contact_force)
    ...
    function eval_con(xv::AbstractArray{T}) where T
        ...
        for i = 1:traj_data.N-1
            ...
            gn = eval_dyn(xn)
            geq[(i-1)*traj_data.num_dyn_eq+1:i*traj_data.num_dyn_eq] = gn[g_eq_selector]
            gineq[(i-1)*traj_data.num_dyn_ineq+1:i*traj_data.num_dyn_ineq] = gn[g_ineq_selector]
        end
...
function trajopt_bilevel(traj_data;opt_tol=1e-6,major_feas=1e-6,minor_feas=1e-6)
    # optimization bounds
    x_L = -1e19 * ones(traj_data.num_x)
    x_U = 1e19 * ones(traj_data.num_x)

    traj_fn = traj_fn_bilevel(traj_data)
